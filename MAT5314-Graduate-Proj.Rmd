...
```{r, fig.align='center', fig.cap = "Pie Chart of Magnitude levels", out.height="4in", out.width="4in"}
...
piechart <- plot_ly(A, labels=~magnitude_level_df$Magnitude_Range, values=~percentages, type="pie")%>% 
  layout(title = list(text="Pie Chart of Magnitude levels", font=list(size=12)),
...
```
\begin{minipage}[t]{0.65\textwidth}
```{r, fig.align='center', fig.cap = "ACF and PACF Plot", out.width='4in', out.height='4in'}
par(mfrow=c(1, 2))
...
\begin{minipage}[t]{0.35\textwidth}
\vspace{8pt}
Generally, we determined the values of p and q in an ARMA model by observing the ACF and PACF plots. Figures(6) represented the ACF and PACF plots. It could be observed that the ACF was relatively high for the first two lags, surpassing the threshold line (blue line), and then gradually decreased. Thus, we inferred that the ACF was exhibiting a tailing pattern. On the other hand, the PACF cut off after lag 1 and subsequent PACF for all lags remained within the threshold line (blue line), indicating a truncated PACF. Consequently, we opted for an AR(1) model.
\end{minipage}
...
```{r, echo=FALSE, fig.align='center', fig.cap = "INARPQX(1) Process Prediction", out.height="4in", out.width="3.5in"}
